...
public class JwtUtil {
    ...
    public static void main(String[] args) {
    	...
    	UserInfo user = null;//new User("1","zhupeijun","123456");
    ...
    public static String buildJsonByUser(UserInfo user)throws JwtException{
        try {
            String subject = new Gson().toJson(user);
            String jwt = JwtUtil.createJWT(Constant.JWT_ID, "zhupeijun", subject, Constant.JWT_TTL);
            return jwt;
    ...
    public static UserInfo getUserByJson(String token) throws JwtException {
        SecretKey key = JwtUtil.generalKey();  //签名秘钥，和生成的签名的秘钥一模一样
        Claims claims = Jwts.parser()  //得到DefaultJwtParser
                .setSigningKey(key)                 //设置签名的秘钥
                .parseClaimsJws(token).getBody();     //设置需要解析的jwt

        return new Gson().fromJson(claims.getSubject(),UserInfo.class);
    }
