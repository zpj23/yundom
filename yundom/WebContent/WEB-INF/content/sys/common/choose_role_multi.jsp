...
function initd(){
	...
	$.ajax({
		   type: "POST",
		   url: "jlRoleInfoAction_getRoleListJson?page"+page_Number+"&rows="+page_Size,
		   async:true,
		   data: "",
...
function chooseOne(){
	...
	for(var m=0;m<rowData.length;m++){
		...
		userid +=rowData[m].id;
		username+=rowData[m].rolename;
		
	}

 	var index = parent.layer.getFrameIndex(window.name);
	parent.choose_role_back(userid,username);
...
<table id="datagrid" fit="true" fitColumns="true"
				...
				pageSize="100" pageList="${ipagelist}"
